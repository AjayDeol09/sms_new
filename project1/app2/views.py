...
def admin_home(request):
    if request.session.has_key('usertype'):
        ...
        if usertype == 'admin':
            ...
            # First time login check — session mein flag set karo
            is_first_login = not request.session.get('visited_before', False)
            request.session['visited_before'] = True  # ab se hamesha False rahega

            # Admin name — session mein naam ho toh use karo, warna email se banao
            admin_name = request.session.get('Name') or email.split('@')[0].capitalize()
    ...
    if(request.session.has_key('usertype')):
        ...
        if(usertype == 'admin'):
            if(request.method == "POST"):
                ...
                # ✅ Check student already selected the course or not
                already_enrolled = st_course.objects.filter(email=email, course_id=course_id).exists()
                if already_enrolled:
                    return render(request, 'add_courses_1.html', {'message': "⚠️ Course already selected! Student cannot enroll in the same course twice."})
